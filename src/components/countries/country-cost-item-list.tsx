...
import type { CountryCostItem } from "@/lib/types";
import {
  formatCurrency,
...
interface Props {
  items: CountryCostItem[];
  defaultCurrency: string;
  onEdit: (item: CountryCostItem) => void;
  onToggleStatus: (item: CountryCostItem) => void;
  onDelete: (id: string) => void;
}

export function CountryCostItemList({
  items,
  defaultCurrency,
  onEdit,
  onToggleStatus,
  ...
  return (
    <div className="space-y-2">
      {items.map((item) => {
        const monthlyCost = getCountryCostItemMonthlyConverted(item, defaultCurrency);
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {COUNTRY_COST_CATEGORY_LABELS[item.category]}
                </span>
              </div>
