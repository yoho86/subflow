...
interface Props {
  ...
  countryId: string;
  item?: CountryCostItem | null;
  onSubmit: (data: CountryCostItemInsert) => Promise<void>;
}

function createEmptyForm(countryId: string): CountryCostItemInsert {
  return {
...
export function CountryCostItemForm({
  open,
  onOpenChange,
  countryId,
  item,
  ...
  const [form, setForm] = useState<CountryCostItemInsert>(createEmptyForm(countryId));
  ...
  useEffect(() => {
    if (item) {
      ...
      setForm(createEmptyForm(countryId));
    }
  }, [item, countryId, open]);

  function update<K extends keyof CountryCostItemInsert>(
    key: K,
    value: CountryCostItemInsert[K]
  ) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{item ? "编辑国家成本项" : "添加国家成本项"}</SheetTitle>
        </SheetHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={form.category}
                onValueChange={(value) =>
                  update("category", value as CountryCostCategory)
                }
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectContent>
                  {Object.entries(COUNTRY_COST_CATEGORY_LABELS).map(([value, label]) => (
